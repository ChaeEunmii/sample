...
import { mockCommonOrder, mockOrderCancelList } from '@mocks/claims';
import styles from './CancelApply.module.scss';

export default function CancelApply() {
  // mock 데이터 할당
  const { topInfo, refund, gift } = mockCommonOrder;
  // 취소 상품 샘플데이터
  const cancelData = mockOrderCancelList as OrderDeliveryData[];

  // 체크박스 상태 관리 (item.id 기준)
  const [checkedItems, setCheckedItems] = useState<Record<string, boolean>>({});

  // 전체 아이템 목록
  const allItems = cancelData[0].sellers.flatMap((s) => s.items);
  ...
  return (
    <Container showBack title="주문 취소 신청">
      <Contents className={styles.layout}>
        ...
        {/* 취소 상품 목록 */}
        <Section title="주문 상품" variant="normal" level="1" flush borderTop>
          <MyOrderGroup
            data={cancelData}
            viewType="detail"
            ...
            bottomSlot={<CheckGiftBox data={gift} status="사은품이 취소될 예정입니다." />}
          />
        </Section>
        {/* 배송비 안내 */}
        <CostInfoSection
