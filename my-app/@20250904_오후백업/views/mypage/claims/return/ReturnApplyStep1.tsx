...
import { ReturnStoreBox, ClaimProdList } from '@/views/mypage/claims/components';
...
// 매장반품 접수안내글
const returnStoreInfoTextList = [
  '매장으로 반품을 선택하신 경우, 구매한 오프라인 매장에의 접수 안내 문자를 받으신 다음, 직접 방문하여 반품을 진행해 주세요.',
  '반품 접수 하신 이후에는 반품 방법을 변경할 수 없습니다.',
];

export default function ReturnApplyStep1() {
  // mock 데이터 가져오기
  const { topInfo, claimApplyItems, claimApplyMultiItems, deliveryDetailItems, refund } =
    mockCommonOrder;
  ...
  const DeliveryGroupListData = [
    {
      ...
      orderData: claimApplyMultiItems[0], //반품상품
    },
    {
      id: 'addr-2',
      infoData: deliveryDetailItems[2], // 회수지
      orderData: claimApplyMultiItems[1], //반품상품
    },
  ];
          ...
          bottomSlot={
            <>
              <div className={styles.method}>
                ...
                />
                <TextList
                  data={isReturnRadio === '2' ? returnStoreInfoTextList : returnInfoTextList}
                  variant="info"
                  className="ncp-mt-s"
                />
              </div>
