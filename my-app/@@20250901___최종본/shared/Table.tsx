...
const TableBody = ({ children, className, visibleCount }: TableBodyProps) => {
  ...
  const renderChildren =
    typeof visibleCount === 'number' ? sortedChildren.slice(0, visibleCount) : sortedChildren;

  return <tbody className={clsx(styles.tbody, className)}>{renderChildren}</tbody>;
};
