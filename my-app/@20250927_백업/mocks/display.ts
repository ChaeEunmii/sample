...
export const mockCurrentLiveTile = {
  ...
  href: '/live/1',
  date: {
    start: new Date().toISOString(), // 현재 시간 (라이브 중)
    end: new Date(Date.now() + 2 * 60 * 60 * 1000).toISOString(), // 2시간 후 종료
  },
};
...
export const mockLiveTiles = [
  ...
  },
  {
    ...
    href: '/live/5',
    date: {
      start: new Date(Date.now() + 48 * 60 * 60 * 1000).toISOString(), // 이틀 후
      end: new Date(Date.now() + 50 * 60 * 60 * 1000).toISOString(), // 이틀 후 2시간 후
    },
  },
];
...
const detTabs = [{ label: '전체' }, ...Array(3).fill({ label: '세카테고리' })];
const subTabs = [...Array(6).fill({ label: '소카테고리', detTabs }), { label: '소카테고리' }];

export const mockCategoryFilterTabs = [
  // { label: '중카테고리' }, // 첫 번째: subTabs 없는경우
  ...Array(5).fill({ label: '중카테고리', subTabs }),
];
