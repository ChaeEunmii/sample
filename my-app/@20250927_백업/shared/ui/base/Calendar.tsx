...
interface BaseProps {
  ...
  maxDate?: Date;
  className?: string;
  tileDisabled?: (args: CalendarTileProperties) => boolean; // true 반환 시 해당 날짜 비활성화
}

// 단일 날짜 선택용 인터페이스
interface SingleCalendarProps extends BaseProps {
  value: Date | null;
...
interface RangeCalendarProps extends BaseProps {
...
export const Calendar = (props: CalendarProps) => {
  ...
  const handleChange = (newValue: Value) => {
    if (dateRange) {
      (onChange as RangeCalendarProps['onChange'])(newValue as [Date | null, Date | null]);
    } else {
  ...
  return (
    <ReactCalendar
      ...
      locale="ko-KR"
      nextLabel={<Icon name="arrowRight" size="xsmall" label="이전" />}
      prevLabel={<Icon name="arrowLeft" size="xsmall" label="다음" />}
      next2Label={null}
      prev2Label={null}
