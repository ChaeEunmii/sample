...
export const FormItem = (({
  ...
  // 첫 번째 field 컴포넌트에 id 부여하되, Stack인 경우 내부 첫 번째 요소에 부여, Chip인 경우 내부 첫 번째 input에 id 부여
  const fieldComponentsWithId = React.Children.map(fieldComponents, (child, index) => {
    ...
    if (index === 0) {
      ...
      const isChip = childType?.displayName === 'Chip';
      ...
      // Chip 처리: Chip 내부 첫 번째 input에 id 부여
      if (isChip) {
        return React.cloneElement(typedChild, { firstInputId: id });
      }
