...
import { A11y, Autoplay, Pagination, FreeMode, Navigation, Zoom } from 'swiper/modules';
...
export const Carousel = ({
  ...
  const modules = [A11y, Autoplay, Pagination, FreeMode, Navigation, Zoom];
  ...
  // Slides 랜더
  const renderSlides = (slideGroup: any[], groupIdx: number) => {
    if (rows) {
      return (
        <>
          {slideGroup.map((slide, slideIdx) => (
            <Fragment key={`${groupIdx}-${slideIdx}`}>{slide}</Fragment>
          ))}
        </>
      );
    }
  ...
  return (
    ...
    >
      <Swiper
        ...
        spaceBetween={boxing ? spaceBetween : undefined}
        ...
        style={
          {
            ...carouselStyles,
            '--carousel-space-between': `${spaceBetween && !boxing ? spaceBetween : 0}px`,
      ...
      >
        {groupedSlides.map((slides, idx) => (
          ...
          >
            {zoom ? (
              <div className="swiper-zoom-container">{renderSlides(slides, idx)}</div>
            ) : (
              renderSlides(slides, idx)
            )}
          </SwiperSlide>
        ))}
      </Swiper>
