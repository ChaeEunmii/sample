...
export interface EmptyProps extends React.HTMLAttributes<HTMLDivElement> {
   ...
   * - inline: tab 등 내용 영역에 최소 높이로 들어갈 때 사용
   */
  variant?: 'minDisplay' | 'inline' | string;
  /** 주요 문구 설정 */
  title?: string;
  /** 보조 문구 설정 */
...
export const Empty = ({
  ...
  // 마진 상단값 업데이트 함수
  const updateMargin = () => {
    // variant = minDisplay | inline일 경우 리턴
    if (variant && ['minDisplay', 'inline'].includes(variant)) return;
  ...
  if (!variant && !isClient) return null;
