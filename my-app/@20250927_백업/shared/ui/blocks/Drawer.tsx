...
interface DrawerProps {
  ...
  /** 뒤로가기 형태 버튼 노출 여부 */
  showBack?: boolean;
  /** 딤 영역 슬롯 (drawer root의 top을 기준으로 80px 떨어진 곳부터 시작) */
...
export const Drawer = ({
  ...
  const { stack, push, pop, addPreShow, preShows } = useDrawer();
  const drawerIdRef = useRef(useId());
  const drawerId = drawerIdRef.current;
  const isFirstDrawer = stack[0] === drawerId;

  const handleClose = () => {
    if (!isFirstDrawer) {
      // 닫기 전 아래(두번째) Drawer를 먼저 보이게 하기 위해 preShows에 추가
      const belowId = stack[stack.length - 2];
      if (belowId) addPreShow(belowId);
      // 0.1초 지연 후 현재 Drawer hide
      setTimeout(() => {
        onClose();
      }, 100);
    } else {
  ...
  const handleDragMove = useCallback(
    (e: MouseEvent | TouchEvent) => {
      ...
      const deltaY =
        placement === 'bottom'
          ? startY.current - clientY // 위로 드래그하면 양수
          : clientY - startY.current; // 아래로 드래그하면 양수
  ...
  const isTopMost = stack[stack.length - 1] === drawerId;
  const isPreShown = preShows.has(drawerId);
  ...
  return ReactDOM.createPortal(
    <FocusTrap>
      <div
        className={clsx(styles.overlay, dimSlot && styles.hasDimSlot)}
        {...overlayProps}
        style={{
          ...(typeof offset === 'number' ? { bottom: `${offset}px` } : {}),
        }}
        onClick={(e) => {
      ...
      >
        {isFirstDrawer && <div className={styles.dimmed} />}
        {dimSlot && (
          <div className={styles.dimSlot} data-state={show ? 'show' : 'hide'}>
            {dimSlot}
          </div>
        )}
        <div
          ...
          style={{
            ...(draggable && height !== null
              ...
              : {}),
            ...(!isFirstDrawer ? { transition: 'none' } : {}),
            opacity: isPreShown || isTopMost ? '1' : '0',
          }}
        >
          {draggable && (
            <div
          ...
          {title && (
            <div className={clsx(styles.header)}>
              {showBack && (
                <IconButton name="back" size="large" onClick={handleClose}>
                  닫기
                </IconButton>
              )}
              <Heading
              ...
              {!draggable && !collapsible && !showBack && (
                <IconButton
                  name="close"
                  size="large"
                  onClick={handleClose}
