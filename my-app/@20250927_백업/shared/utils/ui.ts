...
export const formatNumber = (number: number, unit?: string): string => {
  switch (unit) {
    ...
    case 'userEn':
      return `${new Intl.NumberFormat('en-US').format(number)} Person`;
 ...
 * @param onlyDate 무조건 년월일까지만 출력할지 여부 (기본값: false)
 ...
 * formatDate('202307301530', 'kor', true, true)          // 2023년 07월 30일 (onlyDate 적용)
 * formatDate('20230730153045', 'dot', false, true)       // 2023.7.30 (usePad=false, onlyDate=true)
 */
...
/**
 * 'MM.DD 요일' 형식의 문자열을 변환하는 함수
 * @param date date객체
 * @returns 'MM.DD 요일' 형식의 문자열
 */
export function formatMonthDateDay(date: Date): string {
...
export const calculateBottom = (): number => {
  ...
  const stickyEl = document.querySelector('[data-sticky-state="true"]') as HTMLElement;
...
/**
 * 비디오 파일 여부 체크
 * 확장자 목록: mp4, webm, ogg, mov, avi, wmv, flv, mkv
 * @param src 파일 경로
 * @returns 비디오 확장자일 경우 true, 아니면 false
 */
export const isVideoFile = (src: string) => {
