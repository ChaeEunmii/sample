      ...
      <div className={styles.root}>
        ...
        <ul className={styles.list}>
          {data.map((item) => {
            ...
            const headingId = `collection-${item.id}-label`;
            return (
              <li
                key={item.id}
                ...
                role="button"
                tabIndex={0}
                aria-pressed={isSelected} // 토글 버튼 의미
                aria-labelledby={headingId} // 스크린리더 라벨 연결
                onClick={() => toggleItem(item.id)} // 마우스 클릭
                onKeyDown={(e) => {
                  // 키보드 Enter/Space 지원
              ...
              >
                <div className={styles.side}>
                  ...
                  <div className={styles.infoWrap}>
                    <Heading as="strong" id={headingId} className={styles.label}>
                      {item.label}
                      {item.isLock && <Icon name="lock" size="xsmall" />}
                    </Heading>
                  ...
                  {/* 체크박스는 보조 UI: 버블링 막아 중복 토글 방지 */}
                  <Checkbox
                    label={item.label}
                    size="medium"
                    checked={isSelected}
                    tabIndex={-1} // 체크박스 모양에는 포커스 제외
