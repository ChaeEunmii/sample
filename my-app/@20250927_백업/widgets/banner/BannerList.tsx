...
type DefaultBannerData =
  | Omit<
      BannerProps,
      'align' | 'textInside' | 'ratio' | 'textSize' | 'textSpacing' | 'flush' | 'gem'
    >
...
export const BannerList = ({
  ...
  // 위시리스트 상태 관리
  const getWishlistProps = (bannerId: string) => {
    if (!wishlist) return undefined;

    const isActive = wishlist.activeIds.includes(bannerId);
    return {
      isActive,
      onChange: (bannerId: string, newIsActive: boolean) =>
        wishlist.onToggle(bannerId, newIsActive),
    };
  };
