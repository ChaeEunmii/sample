...
import { BoardAccList, BoardAccListItem, BoardAccListProps } from '@widgets/form/BoardAccList';
...
export interface BoardAccSearchProps {
  ...
  data?: BoardAccListItem[];
  /** 탭 + 탭별 데이터 */
  tabs?: { label: React.ReactNode; subTabs?: TabItem[]; data?: BoardAccListItem[] }[];
  /** 초기 탭 인덱스 */
...
export const BoardAccSearch = ({
  ...
  const [typingText, setTypingText] = useState(defaultQuery); // 입력 중인 검색어 상태
  const [searchText, setSearchText] = useState(defaultQuery); // 검색 실행으로 확정된 검색어 상태
  const [clickedSubTab, setClickedSubTab] = useState(0); // 서브 탭 인덱스
  ...
  // 컴포넌트 초기 렌더 시, 기본 검색어(defaultQuery)가 있으면 즉시 검색어 상태에 반영
  useEffect(() => {
    if (defaultQuery.trim()) setSearchText(defaultQuery.trim());
  }, []);
  ...
  const handleTabChange = (idx: number) => {
    ...
    setClickedSubTab(0); // 상위 탭 바뀌면 서브 탭 초기화
  };
  ...
  const isCurrentTabEmpty = useMemo(() => {
    ...
    const currentTab = effectiveTabs[safeTabIndex];
    if (!currentTab) return true;

    // subTabs 존재하면 subTabs 데이터 모두 합쳐서 확인
    if (currentTab.subTabs && currentTab.subTabs.length > 0) {
      return currentTab.subTabs.every((st) => !st.data || st.data.length === 0);
    }

    return !currentTab.data || currentTab.data.length === 0;
  }, [hasTabs, effectiveTabs, safeTabIndex]);
  ...
  const sourceData = useMemo<BoardAccListItem[]>(() => {
    ...
    // subTabs 존재하면 현재 subTab 인덱스 데이터 반환
    if (currentTab.subTabs && currentTab.subTabs.length > 0) {
      const sub = currentTab.subTabs[clampIndex(clickedSubTab, currentTab.subTabs.length)];
      return sub?.data ?? [];
    }

    // subTabs 없으면 상위 tab data 반환
    return currentTab.data ?? [];
  }, [hasTabs, effectiveTabs, safeTabIndex, clickedSubTab, data]);

  // 모든 탭 데이터를 합친 배열 (검색 시 사용)
  const allTabData = useMemo<BoardAccListItem[]>(() => {
    if (!hasTabs) return data ?? [];
    return effectiveTabs.flatMap((t) =>
      t.subTabs ? t.subTabs.flatMap((st) => st.data ?? []) : (t.data ?? [])
    );
  }, [hasTabs, effectiveTabs, data]);

  /* ----------------------- 검색어 하이라이트 및 필터링 처리 ----------------------- */
  const highlighted = useMemo(() => {
    const q = searchText.trim(); // 검색어
    const lcq = q.toLowerCase(); // 소문자 검색어
    const searchingAll = q.length > 0; // 검색 중 여부

    // 아이템의 label/content/html 중 검색어 포함 여부 확인
    ...
    // 검색 결과 아이템에 하이라이트 적용 및 html/content 변환
    return filtered.map((it) => {
      const next: BoardAccListItem = { ...it }; // 원본 복사
  ...
  }, [sourceData, searchText, hasTabs, allTabData]);
  ...
  const listKey = `list-${hasSearched ? 's' : 'b'}-tab${safeTabIndex}-${searchText}`;
  ...
  // 검색창 초기화: 입력 중/확정된 검색어 리셋 + 탭을 초기 인덱스로 복원
  const handleClear = () => {
    setTypingText('');
    setSearchText('');
    if (hasTabs) {
      ...
      {(!isNoData || hasTabs) && (
        <div className={clsx(styles.search)} role="search">
          <Input
            ...
            clearable={false}
            onChange={(e) => setTypingText((e.target as HTMLInputElement).value)}
            onKeyDown={(e) => {
              if (e.key === 'Escape') {
                e.preventDefault();
                handleClear();
            ...
            suffix={
              // 디자인가이드 : 텍스트 입력 시 검색버튼이 사라지며 검색에 대한 클릭은 키패드에서 엔터키 입력 시 작동
              typingText ? (
                <IconButton name="delete" className={styles.clear} onClick={handleClear}>
                  지우기
                </IconButton>
              ) : (
                <IconButton name="search" aria-label="검색" onClick={runSearch} size="large" />
              )
            }
            size="large"
          />
          ...
          {hasTabs && !hasSearched && (
            <>
              ...
              />
              {effectiveTabs[safeTabIndex]?.subTabs && (
                <Tabs
                  variant="texts"
                  textActiveType="none"
                  data={effectiveTabs[safeTabIndex]!.subTabs!.map((st) => ({
                    label: toPlainLabel(st.label),
                  }))}
                  className={styles.subtab}
                  activeTab={clickedSubTab}
                  onTabChange={(idx) => setClickedSubTab(idx)} // 클릭 시 상태 변경
                />
              )}
            </>
          )}
...
BoardAccSearch.displayName = 'BoardAccSearch';
