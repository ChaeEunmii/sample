...
export const AgreementGroup: React.FC<AgreementGroupProps> = ({
  ...
  const renderType4Item = () => (
    <Collapse variant="section" className={styles.collapse}>
      ...
      <Collapse.Content>
        {data.content && (
          <div className={clsx(styles.termsWrap, styles[`${data.termsClassName}`])}>
            {data.content}
          </div>
        )}
        {renderChildren()}
      ...
      {depth === 0 ? (
        ...
        <li className={clsx(styles[`${data.className}`], data.required && styles.required)}>
          ...
          {isType2 && data.content && (
            <div className={clsx(styles.termsBox, data.termsClassName)}>{data.content}</div>
          )}
          {isType4 && !useCollapse && data.content && (
            <div className={clsx(styles.termsWrap, data.termsClassName)}>{data.content}</div>
          )}
          {!useCollapse && renderChildren()}
