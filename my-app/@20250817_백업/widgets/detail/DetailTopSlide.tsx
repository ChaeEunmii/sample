...
import { Carousel, Flag, Text, Image, Video, ImageViewer } from '@/shared/ui';
    ...
    <>
      <div className={clsx(styles.root)}>
        <Carousel
          ...
          slidesPerView={1}
          pagination="fraction"
          slideTo={currentThumbIdx}
          onSlideChange={setCurrentThumbIdx}
          slides={item.thumbImages.map(
            (img: { src: string; alt: string; poster?: string }, idx: number) => (
              <button key={idx} onClick={() => setIsImageViewerOpen(true)}>
                {isVideoFile(img.src) ? (
                  <Video
                    src={img.src}
                    label={img.alt}
