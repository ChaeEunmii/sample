...
import { MyOrderItemsList, GroupedOrderData } from '@/views/mypage/components/MyOrderItemsList';
  ...
  return (
    <Container showBack title="주문/배송" type="myorder">
      <Contents className={styles.layout}>
        ...
        />
        {/* 주문번호 + 해당 주문번호에 배송타입별로 묶여있는 주문/배송 목록 */}
        {myOrderData.length > 0 ? (
          <>
            {myOrderData.map((orderGroup, index) => (
              <div key={`${orderGroup.orderNumber}-${index}`} className={styles.orderGroup}>
