...
> = {
  ...
  phone: { icon: 'phone', label: '휴대폰 인증' },
  email: { icon: 'email2', label: '이메일 인증' },
  toss: { icon: 'toss', label: '토스 인증', useImage: true, imageUrl: '/images/toss.svg' },
  ...
  (
    ...
    const renderPrefix = () => {
      if (shouldUseImage && icon) {
        return <img src={imageUrl} alt={`${authType} logo`} className={clsx(styles.iconImage)} />;
      }
      return null;
    };
    // 일반 아이콘 설정
    ...
    return (
      <>
        ...
        >
          ...
          {displayText}
          {isRecentLogin && <Tip className={styles.recentTxt}>최근에 로그인 했어요</Tip>}
        </button>
      </>
    );
  }
