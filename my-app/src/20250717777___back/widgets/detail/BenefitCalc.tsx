...
import { Button, Checkbox, Collapse, Line, Text, TextButton } from '@/shared/ui';
...
export const BenefitCalc = ({ data, price }: BenefitCalcProps) => {
  ...
  // 임시데이터
  const hasCouponList = mockHasCouponList; // 회원이 보유하고 있는 쿠폰 리스트
  ...
  /** 쿠폰 받기/보유 랜딩 */
  const renderDownloadableBtn = (item: BenefitItem) => {
    if (!item.isDownloadable) return null;

    const isOwned = hasCouponList.includes(item.id);

    return (
      // TODO: 버튼 사이즈 교체 필요
      <Button size="xsmall" variant="primary" disabled={isOwned}>
        {isOwned ? '보유' : '받기'}
      </Button>
    );
  };
