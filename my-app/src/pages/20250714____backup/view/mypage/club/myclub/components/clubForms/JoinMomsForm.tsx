...
export const JoinMomsForm = ({
  ...
  const renderChip = (index: number, field: keyof ChildInfo, required = false): React.ReactNode => {
    ...
    return (
      <FormItem label={label} required={required} key={`${field}-${index}`}>
        <Chip
          variant="filled"
          size="small"
    ...
    <>
      <Contents>
        ...
        <FormArea type="vertical" gap="3">
          <FormItem label={myClubData.child.label} required>
            <Chip
              variant="filled"
              size="small"
              ...
              <Collapse variant="section" defaultOpen className={styles.collapse}>
                ...
                <Collapse.Content>
                  <FormArea type="vertical" gap="3">
                    ...
                    {/* 이름(애칭) 입력 */}
                    <FormItem label="아이 이름(애칭)" required id={`child-name-${index}`}>
                      <Input
                    ...
                    {/* 아이 생일/출산 예정일 */}
                    <FormItem label="아이 생일/출산 예정일" required id={`child-birthday-${index}`}>
                      <Input
