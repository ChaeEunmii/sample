...
import { periodOptions, mockDepositData } from '@mocks/mypage';
...
export default function DepositMain() {
  ...
  // mock 데이터 변수에 할당
  const summaryData = mockDepositData.summary;
  const historyData = isNoData ? [] : mockDepositData.histories;
  if (isNoData) {
    summaryData.deposit = 0;
  }

  // 기간 정렬
  const [sortBrandValue, setSortBrandValue] = useState('option2');
