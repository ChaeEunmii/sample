...
function renderAmount(type: VoucherType, amount?: number): React.ReactNode | null {
  ...
  return <span className={styles.amount}>{amount}매</span>;
}
...
function renderInfoText(
  ...
  switch (type) {
    ...
    case 'reservation':
      ...
      if (status === 'cancelled') return infos.cancelDate ? format(infos.cancelDate) : null;
      return null;
  ...
  return (
    <>
      <div id={id} className={clsx(styles.root, styles.horizontal, className)} {...props}>
        {/* 썸네일 영역: 이미지와 RSVP 플래그, 상태 오버레이 표시 */}
