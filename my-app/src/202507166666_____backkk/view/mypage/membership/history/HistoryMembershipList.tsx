...
import { Collapse, Heading, Text, Box, InfoList, InfoItem, Line } from '@shared/ui';
      ...
      <div className={styles.layout}>
        {data.map((item, idx) => (
          <div key={item.id ?? idx}>
                ...
                <Box size="3" className={styles.infobox}>
                  <InfoList variant="stacked">
                    ...
                    <InfoItem
                      ...
                      content={
                        <Text weight="semibold">{formatPrice(item.infos.paymentAmount)}</Text>
                      }
                    />
                    <InfoItem
            ...
            {idx < data.length - 1 && <Line color="bg2" />} {/* 마지막 요소 제외 */}
          </div>
        ))}
      </div>
    </>
