...
export const GeneralCTA = ({ setDrawerType, setIsDrawerOpen }: CTACommonProps) => {
      ...
      {isSimplePay ? (
        ...
        <>
          {prodDetailType === 'holidayOnly' && item.useChat && (
            <Button iconOnly="chat" size="large" onClick={() => {}}>
              상담하기
            </Button>
          )}
          {item.isGiftable && (
            <>
              ...
              <Tip
                className={clsx(styles.presentTip, !showTip && styles.hide)}
                placement="top"
                arrowPlace="start"
              >
