...
import styles from './TorderItem.module.scss';

/** 테이블 오더 스타일 */
export type TableOrderItemVriant = 'default' | 'oneLine' | null | undefined | (string & {});
...
// order 아이템
export interface TableOrderItemProps {
  /** 테이블 오더 스타일
   * - default: 왼쪽 이미지 + ProdTitle, ProdPrice간 간격 4px
   * - oneLine:
   */
...
export const TorderItem = ({
  ...
  //썸네일 좌측 상단 플래그 설정
  const getThumbFlag = (item: TableOrderItem) => {
    if (!item.rsvp) return;
    const baseFlags = [];
    if (item.rsvp)
      baseFlags.push({
  ...
  // 아이템이 선택 가능한지 여부 확인
  const isSelectable = (item: TableOrderItem) => {
      ...
      <div className={clsx(styles.itemWrap)}>
        ...
        <div className={clsx(styles.right, !item.image && styles.textMode)}>
          <div className={styles.detail}>
            {variant === 'oneLine' ? (
              <div className={styles.oneLineInfo}>
                <Text as="span" size="4" className={styles.title}>
                  {item.title}
                </Text>
                <Text as="span" size="4" className={styles.line}>
                  {item.quantity}개
                </Text>
                <Text as="span" size="4" className={styles.line}>
              ...
              <>
                {!item.href ? (
                  <div className={clsx(styles.prodTitle, item.disabled && styles.disabled)}>
                  ...
                  <Link
                    ...
                    href={item.href ?? '#'}
                    type="block"
                    className={clsx(styles.prodTitle, item.disabled && styles.disabled)}
                  >
                    <ProdTitle brand={item.brand} title={item.title} />
                  </Link>
                )}
          ...
          {!hidePrice && item.price && item.price?.current !== undefined && (
            <ProdPrice
              className={clsx(styles.price, item.disabled && styles.disabled)}
...
TorderItem.displayName = 'TorderItem';
