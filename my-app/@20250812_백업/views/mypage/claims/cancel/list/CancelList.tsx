...
import { Button, Empty, TextButton, Line } from '@shared/ui';
import { Container, Contents } from '@widgets/layout/Container';
import { TagFilter } from '@widgets/order';
import { GoToHomeBox } from '@views/mypage/order/components/GoToHomeBox';
import { OrderGroup } from '@/views/mypage/components/OrderGroup';
import { GroupedOrderData } from '@/views/mypage/components/OrderGroup';
import { mockClaimsList } from '@mocks/claims';
import styles from './CancelList.module.scss';
...
export default function CancelList() {
  ...
  // mock 데이터 변수에 할당
  const myOrderData = isNoData ? [] : (mockClaimsList as GroupedOrderData[]);
  ...
  const tabMainItems = isNoOrderAndNotFiltered
    ? [{ label: '전체 0' }]
    : [{ label: `전체 10` }, { label: '취소 5' }, { label: '반품 3' }, { label: '교환 2' }];

  return (
    <Container showBack title="취소/반품/교환" type="myorder">
      <Contents className={styles.layout}>
        ...
        <TagFilter
          ...
          drawerTitle="어떤 취소/반품/교환 주문을 찾으세요?"
          isClaims
        />
        {/* 주문번호에 대한 상품목록 구성 반복 */}
