...
interface PaymentMethodsProps {
  ...
  /** 레이아웃 형태 ('collapse' | 'normal' | 'section') */
  ...
  hideHpoint?: boolean;
  /** 현대백화점 카드 숨김 여부 */
  hideHyundaiCard?: boolean;
  /** 글로벌페이 숨김 여부 */
  hideGlobal?: boolean;
  /** 데이터 선택 필수 요소검사("결제 수단을 선택해 주세요. 노출 여부) */
...
// 임시 데이터 (결제수단)
import { paymentMethods, paymentTorderMethods } from './payment';
// Options
...
export const PaymentMethods = ({
  ...
  // 결제수단 선택 라디오 목록 > DOM에서 가장 첫 번째 라디오 value를 읽음
  useEffect(() => {
    if (!radioSelected) {
      // DOM에서 가장 첫 번째 라디오 value를 읽음
      const firstInput = document.querySelector('input[name="controlled"]') as HTMLInputElement;
      if (firstInput) setRadioSelected(firstInput.value);
    }
  }, []);
  ...
  // 일반 + 간편 결제 데이터(torder 타입일 경우,
  // - 회원: 일부 결제 옵션(휴대폰결제, 페이코, 무통장입금, 상품권결제) 제외
  // - 비회원: 신용카드만 노출)
  const filteredPayOptions =
    type === 'torder'
      ? isGuest
        ? // 비회원 → 신용카드만
          payOptions.filter((option) => option.value === 'card')
        : // 회원 → 일부 결제 수단 제외
          payOptions.filter(
            (option) =>
              option.value !== 'phone' &&
  ...
  return (
    <>
      <Section
        ...
        defaultOpen
        defaultSuffix
        className={clsx(styles.paymentMethods, sectionFlush && styles.sectionFlush, className)}
      >
        {/* 카드 결제불가 상태 시 */}
        {type !== 'torder' && isDisabledCard && radioSelected === 'option1' && (
              ...
              {radioSelected === 'option1' && (
                <>
                  <div className={styles.radioCont}>
                    <PaymentMethodSwiper
                      methods={type === 'torder' ? paymentTorderMethods : paymentMethods}
        ...
        {validateMode && (
          <Text size="4" color="alert" indent className={clsx(styles.padded, 'ncp-mt-x6')}>
            결제 수단을 선택해 주세요.
          </Text>
        )}
      </Section>
