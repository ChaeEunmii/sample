...
const flagMap: Record<AlarmStatus, { label: string; color: 'gray3' | 'dark2' }> = {
  ...
  canceled: { label: '입고대기중', color: 'gray3' }, // 해제도 배지는 동일(카드 dim 처리 권장)
      ...
      {!isEmpty && (
        <>
          <Stack type="between">
            <Text
              as="label"
              htmlFor="restock-alram"
              size={!isRestockAlram ? '5' : '7'}
              weight="bold"
            >
              재입고 알림 받기
            </Text>
      ...
      {!isEmpty ? (
        <>
          <OrderItem
            ...
            hidePrice
            isDelete
            className={!isRestockAlram ? styles.disabled : undefined}
          />
