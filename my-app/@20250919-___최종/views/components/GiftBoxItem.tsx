...
import styles from './GiftBoxItem.module.scss';

/**
 *
 * @description 선물함 주문아이템 (OrderItem.tsx 구성) (상태 플래그/버튼/구독일정요약 포함)
 */
...
export function GiftBoxItem({
  ...
  // OrderSlot 슬롯 노출 조건
  const showOrderSlot = !hideStatusBtns;
  ...
  return (
    <>
      ...
      <OrderItem
        ...
        // 가격영역 숨김이면 orderData slot으로 갯수 노출
        orderData={(item) => (
            ...
            <div className={styles.topSlot}>
              {item.orderStatus ? (
                <OrderStatusFlag
                  ...
                  extraFlags={
                    item.orderStatus?.gift?.isSelectionCompleted
                      ? ['selectionCompleted']
                      : undefined
                  }
                />
              ) : null}
              {item.orderStatus?.gift?.receiverName && (
                <div className={styles.receiver}>
                  <Heading className={styles.receiverTit}>받는 분</Heading>
                  <span className={styles.name}>{item.orderStatus.gift.receiverName}</span>
                </div>
              )}
            </div>
          </>
