...
// 기간날짜 제목 + 조건부 표시(showDateTitle)
function renderDateTitle(
  type: VoucherType,
  showDateTitle?: boolean,
  infos?: VoucherCardProps['infos']
) {
  if (!showDateTitle || !infos?.startDate || !infos?.endDate) return null;

  const label =
    type === 'reservation' || type === 'voucher'
      ? '방문 기간'
      ...
      >
        ...
        <CardTag href={href} className={styles.detail}>
          ...
          <div className={styles.bottom}>
            ...
            {/* 기간에 타이틀표시 */}
            {renderDateTitle(type, showDateTitle, infos)}
            {/* 날짜 및 상태 정보 */}
