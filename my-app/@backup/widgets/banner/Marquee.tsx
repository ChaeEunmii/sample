import React, { useEffect, useRef, useState } from 'react';
import clsx from 'clsx';
import styles from './Marquee.module.scss';

interface MarqueeProps {
  children: React.ReactNode;
  /** 속도(px/s): 1초에 number(px)만큼 움직임 */
  speed?: number;
  /** 흐르는방향 (기본:right (좌->우)) */
  direction?: 'left' | 'right';
  /** 높이 지정 */
  height?: number | string;
  /** 복제 횟수 (기본 3) */
  duplicate?: number;
  /** 추가적인 클래스 */
  className?: string;
}

export const Marquee = ({
  children,
  speed = 50,
  direction = 'right',
  className,
  height,
  duplicate = 3,
}: MarqueeProps) => {
  const trackRef = useRef<HTMLDivElement>(null);
  const [duration, setDuration] = useState<number | null>(null);

  useEffect(() => {
    if (!trackRef.current) return;
    // 원본 1세트(content) 너비 측정
    const firstContent = trackRef.current.querySelector<HTMLElement>(`.${styles.content}`);
    if (!firstContent) return;
    const distance = firstContent.scrollWidth;
    // duration = 거리(px) / 속도(px/s)
    setDuration(distance / speed);
  }, [speed, children]);

  const style: React.CSSProperties & { [key: string]: string } = {
    ...(duration != null ? { animationDuration: `${duration}s` } : {}),
    animationPlayState: duration != null ? 'running' : 'paused',
    '--duplicate': String(duplicate), // CSS 변수로 전달
    ...(height ? { height: typeof height === 'number' ? `${height}px` : height } : {}),
  };

  const contents = Array.from({ length: duplicate });

  return (
    <div className={clsx(styles.root, className)}>
      <div ref={trackRef} className={clsx(styles.track, styles[direction])} style={style}>
        {contents.map((_, i) => (
          <div key={i} className={styles.content} aria-hidden={i > 0}>
            {children}
          </div>
        ))}
      </div>
    </div>
  );
};
