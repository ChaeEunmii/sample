...
export const Rsvp_rcmm_01 = ({
  ...
  const [isRccmDrawer, setIsRccmDrawer] = useState(false); // 관심 카테고리 설정 (D)
  ...
  return (
    <Display margin={margin}>
      ...
      {activeIdx === 0 &&
        (products?.length > 0 ? (
          ...
          <Empty
            ...
            buttons={
              !hasKeywords && (
                <Button variant="primary" onClick={() => setIsRccmDrawer(true)}>
                  관심 설정하기
                </Button>
              )
            }
      ...
      {/* 관심 카테고리 선택 (D)*/}
      <InterestCategoryDrawer isOpen={isRccmDrawer} onClose={() => setIsRccmDrawer(false)} />
    </Display>
  );
};
