...
import { ScheduleBox } from '@/views/mypage/subscription/components';
...
import {
  ...
  mockTotalSubsScheduleList,
  mockWaitSubsScheduleList,
      ...
      return (
        <>
          <div
            ...
            style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}
          >
            {mockTotalSubsScheduleList.map((item) => (
              <ScheduleBox
                key={item.id}
                data={item}
                onViewOrder={(id) => {
            ...
            {/* 대기중 */}
            <ScheduleBox
              data={mockWaitSubsScheduleList[0]}
              onViewOrder={(id) => {
                console.log('주문상세 클릭:', id);
              }}
              onTrackDelivery={(id) => {
