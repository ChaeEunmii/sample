...
import { DeliveryChangeDialog, DeliveryRequestEditDialog } from '@/widgets/order';
...
interface DeliveryDetailProps {
  ...
  /** 정보변경 버튼 숨기 여부 */
  hideChangeBtns?: boolean;
  /** 기본 배송지 마크 숨김 여부 */
  hideDefaultMark?: boolean;
  /** 추가적인 클래스 이름 */
  className?: string;
}
      ...
      >
        {isEmpty ? (
          ...
          <>
            <Box size="3" margin="0">
              <AddressInfoBox
                isChangeButton={!hideChangeBtns}
                changeButtonText="배송지 정보 변경" // 변경 버튼 텍스트 수정
                isDefaultAddress={!hideDefaultMark} // 기본 배송지 여부
                disablePosition // 변경 버튼 positoin 해제
                removePadding // 내부 패딩 삭제
                onClick={() => setIsDeliveryChangeDialog(true)}
              />
