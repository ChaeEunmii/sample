...
export const DetailPrice = () => {
  ...
  const { price, unit, isStockView, stock, salePeriod } = item;
    ...
    <>
      <div className={clsx(styles.root)}>
        ...
        <div className={styles.priceCouponWrap}>
          ...
          {/* 쿠폰받기 버튼 */}
          {prodType !== 'showroom' && prodDetailType !== 'subscription' && (
            <Button
              variant="primary"
              size="xsmall"
              suffixIcon="download"
        ...
        {(unit || isStockView) && (
          <Text as="span" size="3" className={styles.unitPrice}>
            {unit && `${unit.label} 당 ${formatPrice(unit.price)}`}
            {unit && isStockView && prodDetailType !== 'subscription' && ` / `}
            {isStockView && prodDetailType !== 'subscription' && `남은수량 ${stock}개`}
          </Text>
        )}
      </div>
