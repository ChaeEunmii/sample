...
export const ReviewFilters = ({
  ...
  const handlePersonaChipChange = (
    values: string[],
    groupData: { label: string; value: string }[]
  ) => {
    // 현재 그룹의 기존 선택값들
    const currentGroupValues = (tempFilterValues.persona || []).filter((value) =>
      groupData.some((option) => option.value === value)
    );

    // 현재 그룹의 all 값 찾기
    const groupAllValue = groupData.find((option) => option.value.startsWith('all_'))?.value;

    // 전체 처리 로직
    let newValues = values;
    if (groupAllValue && values.includes(groupAllValue)) {
      if (!currentGroupValues.includes(groupAllValue)) {
        newValues = [groupAllValue];
      } else if (values.length > 1) {
        newValues = values.filter((v) => v !== groupAllValue);
      }
    } else {
      newValues = values.filter((v) => !v.startsWith('all_'));
    }

    // 다른 그룹의 선택값들을 유지하면서 현재 그룹만 업데이트
    const otherGroupValues = (tempFilterValues.persona || []).filter(
      (value) => !groupData.some((option) => option.value === value)
    );
    const newPersonaValues = [...otherGroupValues, ...newValues];

    setTempFilterValues({
      ...tempFilterValues,
      persona: newPersonaValues,
    });
  };
          ...
          {personaOptions.length > 0 && (
            ...
            >
              {personaOptions.map(({ key, title, data }) => {
                // 각 그룹별로 고유한 all 값을 가진 데이터 생성
                const groupData = data.map((item) =>
                  item.value === 'all' ? { ...item, value: `all_${key}` } : item
                );
                ...
                return (
                  <Chip
                    ...
                    data={groupData}
                    multiple
                    selected={(tempFilterValues.persona || []).filter((value) =>
                      groupData.some((option) => option.value === value)
                    )}
                    onChange={(vals: string[]) => handlePersonaChipChange(vals, groupData)}
                    size="small"
                    columns="auto"
                    aria-label={title}
                  />
