...
export function LiveAlarm({ data, follows }: LiveAlarmProps) {
  ...
  const [allLiveAlarm, setAllLiveAlarm] = useState(false);
      ...
      case 'upcoming':
        return (
          <ButtonArea margin="0" vertical>
            <Button disabled={!isLiveAlarm}>알림 해제</Button>
          </ButtonArea>
        );
      case 'canceled':
        return (
          <ButtonArea margin="0" vertical>
            <Button variant="primary" disabled={!isLiveAlarm}>
              알림 재신청
            </Button>
          </ButtonArea>
        );
  ...
  return (
    <>
      <Stack type="between">
        <Text as="label" htmlFor="live-alram" size="5" weight="bold">
          LIVE 알림 받기
        </Text>
        <Switch
          id="live-alram"
      ...
      <Text size="3" color="gray2">
        LIVE 알림 신청시 해당 상품의 LIVE 시점에 방송 알림을
        <br />
        발송합니다.
      </Text>
      ...
      <Checkbox
        label="모든 LIVE 알림 받기"
        checked={allLiveAlarm}
        onChange={(e) => setAllLiveAlarm(e.target.checked)}
        disabled={!isLiveAlarm}
      />
                ...
                orderTopData={(item) => {
                  ...
                  return (
                    <Text as="span" size="4" color="gray3" weight="medium">
                      방송일 {formatDate(meta?.date, 'dot', true)} 오후 05:00
                    </Text>
                  );
                }}
