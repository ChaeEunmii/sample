...
export const mockProdCards2 = mockProdCards.map(({ button, ...rest }) => ({
  ...rest,
}));
...
// 상품 상세 - 함께 구매 가능한 상품 Dialog, 사은품 대상 상품 Dialog 내 목록
export const mockProdCrossDiscountCase = mockProdCards
  .slice(0, 4)
  .map(({ review, delivery, benefits, ...rest }) => ({
    ...rest,
  }));

// 상품 상세 - 연관상품 영역
export const mockOthersViewCase = mockProdCards.map(({ emblem, badge, button, ...rest }) => ({
  ...rest,
}));

// 상품 상세 - 연관상품 영역 목록
export const mockRelatedProdList = [
  {
    title: '다른 사람이 함께 본 상품',
    data: mockOthersViewCase.map((item) => ({ ...item, ad: true })),
  },
  {
    title: '다른 사람이 함께 구매한 상품',
    data: mockOthersViewCase,
  },
];

export const mockSubscriptionRelatedProdList = [
  {
    title: '다른 사람이 함께 본 상품',
    data: mockOthersViewCase.map((item) => ({ ...item, ad: false })),
  },
  {
    title: '다른 사람이 함께 구매한 상품',
    data: mockOthersViewCase.map((item) => ({ ...item, ad: false })),
...
// 랭킹 목록
export const mockProdRanking = mockProdCards
  .slice(0, 10)
  .map(({ review, delivery, benefits, button, ad, ...rest }: any, index) => ({
    ...rest,
    ranking: {
      rank: index + 1,
      change: index % 4 === 0 ? 'up' : index % 4 === 1 ? 'down' : index % 4 === 2 ? 'same' : 'new',
      changeValue:
        index % 4 === 0
...
// 딜 상품 목록
export const mockProdDeal = mockProdCards.map(
  ({ review, delivery, benefits, button, badge, emblem, ...rest }) => ({
    ...rest,
    ad: false,
  })
);
...
// 체험단 목록
export const mockProdPromo = mockProdCards
  .slice(0, 3)
  .map(({ review, delivery, badge, emblem, benefits, button, ad, ...rest }: any, index) => ({
    ...rest,
    winner: 50,
    price: {
      current: 129000,
