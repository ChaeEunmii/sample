...
import {
  mockCommonOrder,
  mockToHomeTorderDetailList,
  mockTableInfoData,
  mockTableStoreInfoData,
  mockRefundInfoData2,
  mockPaymentApprovalData2,
  mockToHomeTorderPayment,
} from '@mocks/history';
import styles from './OrderDetail.module.scss';

export default function OrderDetailTorder() {
  // mock 데이터 가져오기
  const { orderer, topInfo } = mockCommonOrder;

  // 주문상품
  const orderData = mockToHomeTorderDetailList as OrderDeliveryData[];

  return (
    <Container showBack title="주문 상세 정보 - 테이블오더">
      <Contents className={styles.layout}>
        ...
        {/* 테이블오더 매장정보 */}
        <TableOrderStoreInfo data={mockTableStoreInfoData} />
        {/* 테이블정보 */}
        <TableInfo title="테이블정보" data={mockTableInfoData} />
        {/* 수령자 정보 */}
        <OrdererInfo title="수령자 정보" data={{ ...orderer, email: undefined }} />
        {/* 매장 주문취소 환불 */}
        <BoxInfo title="매장 주문취소 환불" data={{ content: '결제수단으로 환불' }} />
        {/* 일부 메뉴 품절 시 수령 여부 */}
        <BoxInfo title="일부 메뉴 품절 시 수령 여부" data={{ content: '품절메뉴 제외하고 받기' }} />
        {/* 결제 및 할인정보 */}
        <PaymentInfo title="결제 및 할인정보" data={mockToHomeTorderPayment} />
        {/* 결제수단 & 승인내역 */}
