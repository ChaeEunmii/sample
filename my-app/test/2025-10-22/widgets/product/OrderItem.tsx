...
import { ProdPrice, ProdTitle, SubscriptionStatus } from '@widgets/product';
...
interface OrderItemListProps<T extends OrderItemType = OrderItemType> {
  ...
  itemWrapGap?: '0' | '16' | '24';
  /** currentPrice 가격 스타일
   * - primary(13px, #222, semibold)
   * - secondary(13px, #8c8c8c, semibold)
   * */
  priceVariant?: boolean | string;
  /** 라인으로 구분되는 스타일 */
  lineDivider?: boolean;
  /** 썸네일 라벨 숨김 여부 */
    ...
    <>
      <ul
        className={clsx(
          ...
          disableList && styles.disableList,
          className
        )}
      >
        {items.map((item, index) => {
          ...
          const labelDefs = [
            ...
            { label: '품절취소', show: item.orderStatus?.status === 'giftbox_soldout_cancel' },
            { label: '접수취소', show: item.orderStatus?.status === 'giftbox_apply_cancel' },
          ] as const;
          ...
          const showTotalMark =
            showTotalPrice &&
            (item.orderStatus?.trip?.type === 'air' || item.orderStatus?.trip?.type === 'asp');
          ...
          return (
            <li
              ...
              className={clsx(
                ...
                selectMode === 'single' && styles.cursor
              )}
            >
              ...
              <div className={clsx(styles.wrap)}>
                {selectable && !(hideCheckbox && isMultiDelivery) && (
                  <>
                    {selectMode === 'single' ? (
                      <Radio
                        name="selectProdItem"
                        label={item.title ? `${item.title} 선택` : '선택'}
                        hideLabel
                        value={item.id}
                ...
                <div
                  className={clsx(styles.itemWrap, itemWrapGap && styles[`gap${itemWrapGap}`])}
                  role={selectMode === 'single' ? 'radio' : 'checkbox'}
                  aria-checked={
                    selectMode === 'single' ? selectedItem === item.id : !!checkedItems?.[item.id]
                  }
                  onClick={() => {
                    if (selectable && !item.href) {
                      const checked = !checkedItems?.[item.id];
                      onSelectItem?.(item.id, checked);
                    }
                  }}
                  onKeyDown={(e) => {
                    if ((e.key === 'Enter' || e.key === ' ') && selectable && !item.href) {
                      e.preventDefault();
                      const checked = !checkedItems?.[item.id];
                      onSelectItem?.(item.id, checked);
                    }
                  ...
                  <div className={styles.itemInfo}>
                    ...
                    {item.image && (
                      ...
                      >
                        ...
                        {item.options && item.delivery?.status !== null && !isMultiDelivery && (
                          <TextButton
                            size="1"
                            variant="underline"
                    ...
                    <div className={clsx(styles.right, priceVariant && styles.gap4)}>
                      ...
                      {(orderDetailData || item.orderStatus) && (
                        <>
                          {orderDetailData?.(item)}

                          {/* 정기구독 케이스일 때만 */}
                          {item.orderStatus?.orderCase === 'subscription' && (
                            <SubscriptionStatus orderStatus={item.orderStatus} />
                          )}
                        </>
                      )}
                      ...
                      {!isMultiDelivery && !isVerticalCenter && !hidePrice && (
                        <>
                          <div
                            className={clsx(
                              showOrderCount && styles.pricebox,
                              showTotalMark && styles.showTotalMark
                            )}
                          >
                            {showTotalMark && <span className="ncp-blind">총</span>}
                            {!isOrderGift &&
                              !isVerticalCenter &&
                              !hidePrice &&
                              item.price &&
                              item.price?.current !== undefined && (
                                <ProdPrice
                                  ...
                                  variant={
                                    // priceVariant가 true면 secondary
                                    priceVariant === true
                                      ? 'secondary'
                                      : // priceVariant가 'primary' 문자열이면 primary
