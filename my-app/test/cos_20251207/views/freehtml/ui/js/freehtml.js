...
// ========== 개인정보처리방침 : UI 아코디언 (.ui_accord_*) ========== //
export function handleUiAccordion(root = document) {
  // 아코디언 내용 모두 닫기 (초기 상태)
  const contents = root.querySelectorAll('.ui_accord_content');
  contents.forEach((content) => {
    content.style.display = 'none';
  });

  const toggles = root.querySelectorAll('.ui_accord_toggle');

  toggles.forEach((btn) => {
    if (btn.__bound_ui_accord) return; // 중복 바인딩 방지
    btn.__bound_ui_accord = true;

    const li = btn.closest('li');
    if (!li) return;

    const accordionContent = li.querySelector('.ui_accord_content');
    const head = btn.closest('.head');
    const textEl = btn.querySelector('.ui_accord_text');

    btn.addEventListener('click', function (event) {
      event.preventDefault();
      if (!accordionContent) return;

      const isVisible = getComputedStyle(accordionContent).display !== 'none';
      ...
      if (isVisible) {
        ...
        if (textEl && openText !== null) {
          textEl.innerHTML = openText;
        }

        if (head) {
          head.classList.remove('btn_open');
          head.classList.add('btn_close');
        }
      } else {
        // 열기
        ...
        if (textEl && closeText !== null) {
          textEl.innerHTML = closeText;
        }

        if (head) {
          head.classList.remove('btn_close');
          head.classList.add('btn_open');
        }
      }
    });
  });
}
// ========== 개인정보처리방침 : UI 모달 (.ui_modal_*, .lay_pop) ========== //
let uiModalResizeBound = false;

function centerUiModal(root) {
  const pops = root.querySelectorAll('.lay_pop');

  pops.forEach(function (pop) {
    if (getComputedStyle(pop).display === 'none') return;

    if (!pop.style.position) {
...
export function handleUiModal(root = document) {
  const modalWraps = root.querySelectorAll('.ui_modal_wrap');
  const dim = root.querySelector('.ui_modal_dim');
  const layPops = root.querySelectorAll('.lay_pop');
  ...
  layPops.forEach(function (pop) {
    pop.style.display = 'none';
  });

  function openPopup(popupId) {
    const popup = root.querySelector(popupId);
    if (!popup) return;
  ...
  // 팝업 오픈 버튼 (.btn_pop_open)
  const openButtons = root.querySelectorAll('.btn_pop_open');
  openButtons.forEach(function (btn) {
    if (btn.__bound_ui_modal_open) return;
    btn.__bound_ui_modal_open = true;
  ...
  // dim 클릭 시 닫기
  if (dim && !dim.__bound_ui_modal_dim) {
    dim.__bound_ui_modal_dim = true;
    dim.addEventListener('click', function () {
      closeAllPopups();
    });
  }
  // 닫기 버튼 (.ui_modal_close) 클릭 시 닫기
  const closeButtons = root.querySelectorAll('.ui_modal_close');
  closeButtons.forEach(function (btn) {
    if (btn.__bound_ui_modal_close) return;
    btn.__bound_ui_modal_close = true;
  ...
  // 리사이즈 때도 중앙 정렬 유지 (한 번만 바인딩)
  if (!uiModalResizeBound) {
    uiModalResizeBound = true;
    window.addEventListener('resize', function () {
      centerUiModal(document);
    });
  }
}
...
export function initCommon(root = document) {
  ...
  handleUiAccordion(root); // 개인정보처리방침 아코디언
  handleUiModal(root); // 개인정보처리방침 모달
}
