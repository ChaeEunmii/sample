...
export default function TagFilterSetDrawer({
  ...
  onApplyFilters,
  hideTopInfo = false,
  desc = '최대 5년간의 구매 이력을 확인할 수 있습니다.',
  periodData,
  multiple = false,
  ...
  /** 선택 상태관리 */
  const [filters, setFilters] = useState<Record<CatalogKey, string | string[]>>(INITIAL_FILTERS);

  // Chip onChange 공통 (string | string[] 대비)
  const onChangeSingle = useCallback(
    (key: CatalogKey) => (v: string | string[]) => setFilters((prev) => ({ ...prev, [key]: v })),
    []
  );
  ...
  const isTripMode = effectiveMode === 'trip';
  ...
  return (
        ...
        <>
          ...
          <Button
            ...
            onClick={() => {
              ...
              const payload: { range: DateRange } & Partial<Record<CatalogKey, string | string[]>> =
                {
                  range,
                };
    ...
    >
      <div className={styles.top}>
        {!onlyCalendar && (
          <Input
            title={isReservationMode ? '매장명' : isTripMode ? '여행명' : '주문한 상품명'}
            placeholder={
              isReservationMode
                ? '매장명으로 검색하세요'
                : isTripMode
