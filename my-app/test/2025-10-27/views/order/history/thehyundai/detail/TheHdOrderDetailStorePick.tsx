...
export default function TheHdOrderDetailStorePick() {
  // mock 데이터 가져오기
  const { orderer, topInfo, paymentApproval, expectedPoint, refund, payment } = mockCommonOrder;

  // 주문상품
  const orderData = mockOrderDetailItemsCase as OrderDeliveryData[];
  // 스토어픽 오더 샘플 (mockOrderDetailList)
      ...
      orderData: {
        ...
        sellers: storepickOrder.sellers.map((seller) => ({
          ...
          items: seller.items.map((item) => ({
            ...item,
            orderStatus: { ...item.orderStatus, status: 'claim_return', noReview: true },
          })),
        })),
