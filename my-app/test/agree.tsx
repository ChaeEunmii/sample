...
export const AgreementGroup: React.FC<AgreementGroupProps> = ({
  ...
  useEffect(() => {
    ...
    // 모든 자식이 체크되었는지 확인
    const allChildrenChecked = validChildren.every(
      (child) => allStates[child.id]
    );
    ...
    if (
      allChildrenChecked &&
      !checked &&
      !linkedChecks?.some((r) => r.targets.includes(data.id))
    ) {
    ...
    if (
      ...
      !linkedChecks?.some(
        (r) =>
          r.targets.includes(data.id) && r.triggers.some((tg) => allStates[tg])
      )
    ) {
  ...
  const showRequiredText =
    validateMode &&
    ((readOnlyMode && isTopLevel && !checked) ||
      (!readOnlyMode && hasUncheckedRequired(data)));
      ...
      {depth === 0 ? (
        ...
        <div className={clsx(styles.root, styles[variant], className)}>
          ...
          {showRequiredText && (isType4 || showBottomRequired) && (
            <Text
              ...
              className={clsx(
                styles.requiredTxt,
                showBottomRequired && styles.bottomRequired
              )}
            >
              {requiredText ? requiredText : "모든 약관에 동의해 주세요."}
            </Text>
          )}
        </div>
      ) : (
        ...
        >
          ...
          {isType2 && data.content && (
            <div className={styles.termsBox}>{data.content}</div>
          )}
