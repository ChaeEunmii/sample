...
import BoardAccList, {
  type BoardAccListItem,
  type BoardAccListProps,
} from "./BoardAccList";
...
export interface BoardAccSearchProps {
  ...
  noDataSlot?:
    | React.ReactNode
    | ((tabIndex: number, reset: () => void) => React.ReactNode);
  /** 전달할 추가 props */
  ...
  if (React.isValidElement<{ children?: React.ReactNode }>(node)) {
    ...
    const kids =
      "children" in el.props
        ? highlightNode(el.props.children, q, kp)
        : el.props?.children;
  ...
  const highlighted = useMemo(() => {
    ...
    return filtered.map((it) => {
      ...
      if (typeof it.html === "string" && it.html.length > 0) {
        ...
        next.content = q
          ? highlightNode(it.content, q, `cnt-${id}`)
          : it.content;
        next.html = undefined;
      }
      return next;
      ...
      {(!isNoData || hasTabs) && (
        <div className={clsx(styles.search)} role="search">
          <Input
            ...
            clearable={false}
            onChange={(e) =>
              setTypingText((e.target as HTMLInputElement).value)
            }
            onKeyDown={(e) => {
              if (e.key === "Escape") {
                e.preventDefault();
