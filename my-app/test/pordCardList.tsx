...
interface ProdCardListProps {
  ...
  caption?:
    | string
    | React.ReactNode
    | ((item: ProdCardProps, index: number) => React.ReactNode);
  /** 컨텐츠영역 추가 슬롯 */
  contSlot?: (item: ProdCardProps, index: number) => React.ReactNode;
  /** 프리모드 여부(swiper) */
  freeMode?: boolean;
}
...
export const ProdCardList = ({
  ...
  const renderCard = (product: ProdCardItemProps, idx: number) => {
    ...
    const wishlistProps = showWishlist
      ? getWishlistProps(product.id)
      : undefined;
    ...
    return (
      <>
        <Carousel
          ...
          spaceBetween={
            (gutter && typeof gutter === "number") ?? cardType === "vertical"
              ? 8
              : 12
          }
          loop={loop}
