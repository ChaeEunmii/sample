import { ButtonArea, Button, Text, TextList } from '@/shared/ui';
...
const buttonConfigs: ButtonConfig[] = [
  // --- list view configs ---
  {
    ...
    match: (orderCase) => orderCase === 'voucher' || orderCase === 'ecoupon', //이용권&e쿠폰
  ...
  {
    viewType: 'list',
    status: ['cancel_apply_digital'],
    match: (orderCase) => orderCase === 'ecoupon', //e쿠폰
    render: (item, onButtonClick) => (
      <ButtonArea className={styles.btns}>
        <Button variant="tertiary" onClick={() => onButtonClick?.('viewDetails', item)}>
          상세조회
  ...
  },
  {
    ...
    status: ['order_completed', 'payment_completed'],
    match: (orderCase) =>
      orderCase === 'experience' || orderCase === 'purchaseGift' || orderCase === 'zeroPrice',
    render: () => null,
  },
  {
    viewType: 'list',
  ...
  {
    viewType: 'list',
    status: ['return_requested_digital', 'return_completed_digital'],
    match: (orderCase) => orderCase === 'voucher' || orderCase === 'ecoupon', //이용권&e쿠폰
    render: (item, onButtonClick) => (
      <ButtonArea className={styles.btns}>
        <Button variant="tertiary" onClick={() => onButtonClick?.('viewDetails', item)}>
  ...
  },
  {
    viewType: 'detail',
    status: ['payment_completed'],
    match: (orderCase) => orderCase === 'voucher', //이용권
    render: (item, onButtonClick) => (
      <ButtonArea className={styles.btns}>
  ...
  },
  {
    ...
    match: (orderCase) => orderCase === 'ecoupon', //e쿠폰
    render: (item, onButtonClick) => {
      const isCancelable = item.orderStatus?.isCancelable ?? false; // 취소가능상태
      return (
        <ButtonArea className={styles.btns}>
          {isCancelable && (
            <Button variant="tertiary" onClick={() => onButtonClick?.('cancelApply', item)}>
              취소요청
            </Button>
          )}
          {!isCancelable && (
  ...
  {
    viewType: 'detail',
    status: 'trip_completed_no_review',
    render: (item, onButtonClick) => (
      <ButtonArea className={styles.btns}>
        <Button onClick={() => onButtonClick?.('writeReview', item)}>리뷰쓰기</Button>
      </ButtonArea>
  ...
  {
    viewType: 'detail',
    status: ['trip_payment_completed'],
    render: (item, onButtonClick) => {
      // 여행확정 여부에 따라 취소하기 버튼 노출
      const isTripConfirmed = item.orderStatus?.trip?.isTripConfirmed ?? false;
      return (
        <>
          {isTripConfirmed && (
            <div className={styles.bottom}>
              ...
              <TextList
                data={[
                  '예약취소를 요청하신 경우에도 환불 규정에 따라 취소 및 환불이 불가능할 수 있습니다. 요청 전 반드시 여행자의 환불 규정을 확인해주세요. ',
                  '취소완료 후 환불은 결제수단에 따라 최대 7일 정도 기간이 소요될 수 있습니다.',
                ]}
                variant="info"
                className="ncp-mt-m"
              />
  ...
  {
    viewType: 'detail',
    status: ['return_completed_digital'],
    match: (orderCase) => orderCase === 'ecoupon', //e쿠폰
    render: () => null,
  },
  {
