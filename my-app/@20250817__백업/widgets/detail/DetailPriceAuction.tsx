...
export function useAuctionPhase(item: any): AuctionPhaseResult {
  ...
  const isPurchasePeriod =
    remainToPurchaseStart === '00:00:00' && remainToPurchaseEnd !== '00:00:00' && isBidEnded;
  const highBidPrice =
    Array.isArray(item.bidders) && item.bidders.length > 0
      ? Math.max(...item.bidders.map((bid: BiddersType) => bid.value))
      : item.price;
...
export const DetailPriceAuction = () => {
  ...
  const currentPrice =
    Array.isArray(item.bidders) && item.bidders.length > 0
      ? Math.max(...item.bidders.map((bid: BiddersType) => bid.value))
      : item.price; // 입찰된 가격이 없으면 시작가
  const increase = currentPrice - price;
  const percent = price > 0 ? (increase / price) * 100 : 0;
      ...
      <div className={clsx(styles.root, styles.auction)}>
        ...
        <div className={clsx(styles.auctionPrices)}>
          ...
          <div className={styles.increaseWrap}>
            ...
            {/* 상승률 */}
            {price && increase > 0 && !isPerchaseURL && (
