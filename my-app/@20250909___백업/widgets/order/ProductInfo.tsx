...
interface ProductInfoProps {
  ...
  /** 쿠폰 변경 버튼 노출 여부(기본: false) */
  hideCouponChangeButton?: boolean;
}
...
export const ProductInfo = ({
  ...
  const [collapseActive, setCollapseActive] = useState<Record<string, boolean>>({}); // collapse
  const handleCollapse = (id: string, active: boolean) => {
    setCollapseActive((prev) => ({
      ...prev,
      [id]: active,
    }));
  };
  ...
  return (
    <>
      ...
      >
          ...
          {data.map((order, index) => (
            <Collapse key={order.id} variant="line" defaultSummary className={styles.prodSection}>
              <Collapse.Control
                ...
                handleClick={(active) => handleCollapse(order.id, active)}
              >
                  ...
                  return (
                    <React.Fragment key={seller.id}>
                      ...
                      <OrderItem
                        ...
                        priceVariant
                        showOrderCount
                        orderData={(item: ProductItemType) => {
                        ...
                        orderSlot={(item: ProductItemType) => (
                          <>
                            {(item.gift ||
                              item.delivery?.date ||
                              !isPresentMulti ||
                              ...
                              <>
                                <div className={styles.slotBox}>
                                  ...
                                  {item.orderRequest && !isPresentMulti && (
                                  ...
                                  {item.notice && (
                                    <Text size="4" color="alert" indent className="ncp-mt-x0">
                                      {item.notice}
                                    </Text>
                                  )}
                                </div>
        ...
        {!hideCouponChangeButton && (
          <Button
            size="medium"
            variant="secondary"
            onClick={() => setIsCouponChangeDialogOpen(true)}
          >
            쿠폰 변경
          </Button>
        )}
