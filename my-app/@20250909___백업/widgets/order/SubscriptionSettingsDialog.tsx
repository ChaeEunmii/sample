    ...
    >
      ...
      <FormArea type="vertical" gap="1">
        {chipItems.map((item, index) => {
          const selected = selectedValues[item.name];
          const isPeriodLocked = isChange && item.name === 'period';

          // isChange + period인 경우에만 선택값으로 옵션을 필터링
          const optionsToRender = isPeriodLocked
            ? item.options.filter((o) =>
                Array.isArray(selected) ? selected.includes(o.value) : o.value === selected
              )
            : item.options;
