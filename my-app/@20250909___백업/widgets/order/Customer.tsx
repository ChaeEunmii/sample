'use client';
import { useState } from 'react';
import { Button, FormArea, FormItem, Input, Text } from '@/shared/ui';
import { Section } from '@/shared/ui/blocks/Section';
import styles from './Customer.module.scss';

interface CustomerInfoProps {
  type?: 'torder' | 'reservation';
  title?: string;
  /** Section 컴포넌트 스타일(기본: collapse) */
  sectionVariant?: 'collapse' | 'normal' | 'section';
  /** suffix 사용 여부(기본: false) */
  hideSuffix?: boolean;
}

export const CustomerInfo = ({
  type,
  title,
  sectionVariant = 'collapse',
  hideSuffix = false,
}: CustomerInfoProps) => {
  // 상태 관리
  const [nameValue, setNameValue] = useState(
    type === 'torder' || type === 'reservation' ? '김현대' : '김*대'
  ); // 이름
  const [phoneValue, setPhoneValue] = useState(
    type === 'torder' || type === 'reservation' ? '010-1234-5678' : '010-****-5678'
  ); // 전화번호
  const [isVisible, setIsVisible] = useState(true); // 마스킹 해제 버튼

  const handleClick = () => {
    setNameValue('김현대');
    setIsVisible(false); // 클릭하면 사라짐
  };

  return (
    <>
      <Section
        title={title ? title : '주문자 정보'}
        suffix={
          !hideSuffix && (
            <>
              <Text as="span" size="5">
                김*대
              </Text>
              <Text as="span" size="5">
                010-****-5678
              </Text>
            </>
          )
        }
        variant={sectionVariant}
        className={styles.customer}
      >
        <FormArea type="vertical">
          <FormItem
            label="이름"
            error={type !== 'reservation' ? '이름을 입력해주세요.' : undefined}
            required={type !== 'reservation'}
            className={styles.stack}
          >
            {/* 에러인 경우 invalid 속성 추가 */}
            <Input
              placeholder="이름을 입력해 주세요"
              size="large"
              value={nameValue}
              onChange={(e) => setNameValue(e.target.value)}
              className={styles.input}
              disabled={type === 'reservation'}
            />
            {isVisible && type !== 'torder' && type !== 'reservation' && (
              <Button
                variant="primary"
                size="large"
                onClick={handleClick}
                className={styles.formButton}
              >
                마스킹 해제
              </Button>
            )}
          </FormItem>
          <FormItem
            label="전화번호"
            error={type === 'torder' ? '전화번호를 입력해주세요.' : undefined}
            required={type === 'torder'}
          >
            <Input
              placeholder="전화번호를 입력해 주세요"
              size="large"
              value={phoneValue}
              onChange={(e) => setPhoneValue(e.target.value)}
              disabled={type !== 'torder'}
            />
          </FormItem>
          {type === 'torder' ? (
            <FormItem label="위치" required>
              <Input size="large" value="압구정본점 88번 테이블" disabled />
            </FormItem>
          ) : type !== 'reservation' ? (
            <>
              <FormItem label="이메일">
                <Input
                  placeholder="이메일을 입력해 주세요"
                  size="large"
                  defaultValue="hy**dai@hyundai.com"
                  disabled
                />
              </FormItem>
              <FormItem label="회사명">
                <Input placeholder="회사명을 입력해 주세요" size="large" />
              </FormItem>
            </>
          ) : null}
        </FormArea>
      </Section>
    </>
  );
};
CustomerInfo.displayName = 'CustomerInfo';
