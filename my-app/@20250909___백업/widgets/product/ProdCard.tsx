  ...
  return (
    ...
    >
      ...
      <CardTag href={href} className={styles.thumb} id={id} check={check}>
        <Image src={adult === 'hide' ? '/images/img_adult_hide.png' : image.src} alt={image.alt} />
        {flagSlot}
        ...
        {ad && <Flag data={{ color: 'black30', label: 'AD' }} radius="tl" className={styles.ad} />}
      </CardTag>
      ...
      <div className={styles.content}>
        <CardTag href={href} className={styles.detail} id={id} check={check}>
          ...
          {normalized.benefits.length > 0 && (
            <Flag data={normalized.benefits} color="gray" className={styles.benefit} />
          )}
          {normalized.review && (
            <Text size="3" className={styles.review}>
              <Icon name="rating" size="xsmall" />
              <b>{normalized.review.rating}</b> ({formatNumber(normalized.review.count)})
            </Text>
          )}
        </CardTag>
