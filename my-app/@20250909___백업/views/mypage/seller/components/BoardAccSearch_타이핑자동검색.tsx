...
export default function BoardAccSearch({
  ...
  // typingText 변경 시, 일정 시간 지난 뒤에 갱신
  const DEBOUNCE_MS = 150;
  const [debouncedText, setDebouncedText] = useState(typingText);
  useEffect(() => {
    const t = setTimeout(() => setDebouncedText(typingText), DEBOUNCE_MS);
  ...
  const highlighted = useMemo(() => {
    const q = debouncedText.trim(); // 검색어
  ...
  }, [sourceData, debouncedText, hasTabs, allTabData]);

  // 검색 결과 개수
  const total = highlighted.length;
  // 검색 여부 (확정된 검색어 기준)
  const hasSearched = debouncedText.trim().length > 0;

  // 리스트 리렌더링 키 (탭 인덱스를 포함해 탭 전환 시 초기화)
  const listKey = `list-${hasSearched ? 's' : 'b'}-tab${safeTabIndex}-${debouncedText}`;
